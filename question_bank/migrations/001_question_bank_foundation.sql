...
-- Backward-compatible with the existing MySQL schema.
-- Manual and extracted questions share the same canonical question bank.
-- Existing manual questions remain VERIFIED; extracted questions should be created as PENDING.
-- This migration is intended to be run once. Existing deployments may safely re-run it
-- only after the migration marker check below confirms it has already completed.
...
-- Stop cleanly if this migration has already been applied.
SET @qb_migration_applied := (
    SELECT COUNT(*) FROM schema_migrations
    WHERE migration_id = '001_question_bank_foundation'
);

-- MySQL has no portable conditional ALTER block outside stored programs, so the
-- migration uses a temporary stored procedure and INFORMATION_SCHEMA checks.
DELIMITER $$
CREATE PROCEDURE apply_question_bank_foundation()
BEGIN
    IF @qb_migration_applied = 0 THEN
        IF NOT EXISTS (
        ...
        IF NOT EXISTS (
            ...
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'questions' AND INDEX_NAME = 'idx_questions_source_type'
        ) THEN
            ALTER TABLE questions ADD INDEX idx_questions_source_type (source_type);
        END IF;

        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'questions' AND INDEX_NAME = 'idx_questions_verification_status'
        ) THEN
            ALTER TABLE questions ADD INDEX idx_questions_verification_status (verification_status);
        END IF;

        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'questions' AND INDEX_NAME = 'idx_questions_canonical'
        ) THEN
            ALTER TABLE questions ADD INDEX idx_questions_canonical (canonical_question_id);
        END IF;
        ...
        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'question_assets' AND COLUMN_NAME = 'page_number'
        ) THEN
            ALTER TABLE question_assets ADD COLUMN page_number INT NULL;
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'question_assets' AND COLUMN_NAME = 'bbox_json'
        ) THEN
            ALTER TABLE question_assets ADD COLUMN bbox_json TEXT NULL;
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'question_assets' AND COLUMN_NAME = 'asset_role'
        ) THEN
            ALTER TABLE question_assets ADD COLUMN asset_role VARCHAR(50) NULL;
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'question_assets' AND COLUMN_NAME = 'source_asset_id'
        ) THEN
            ALTER TABLE question_assets ADD COLUMN source_asset_id VARCHAR(70) NULL;
        END IF;
        IF NOT EXISTS (
            SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'question_assets' AND INDEX_NAME = 'idx_question_asset_page'
        ) THEN
            ALTER TABLE question_assets ADD INDEX idx_question_asset_page (question_id, page_number);
        END IF;

        -- Existing manually-created questions are already trusted by the current app.
        UPDATE questions SET source_type = 'MANUAL' WHERE source_type IS NULL OR source_type = '';
        UPDATE questions SET verification_status = 'VERIFIED' WHERE verification_status IS NULL OR verification_status = '';

        INSERT INTO schema_migrations(migration_id)
        VALUES ('001_question_bank_foundation');
    END IF;
END$$
DELIMITER ;

CALL apply_question_bank_foundation();
DROP PROCEDURE apply_question_bank_foundation;
