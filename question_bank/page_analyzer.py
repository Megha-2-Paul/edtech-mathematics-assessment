...
def _question_range(text: str) -> tuple[int | None, int | None]:
    for pattern in QUESTION_RANGE_PATTERNS:
        ...
        if match:
            return int(match.group(1)), int(match.group(2)) if match.lastindex and match.lastindex >= 2 else int(match.group(1))
...
def analyze_page(page: Any, page_number: int) -> dict[str, Any]:
    ...
    english_score = _english_score(text)
    ...
    if english_score >= 0.6 and is_question_page:
        language = "english"
    elif english_score < 0.4 and is_question_page:
    ...
    return {
        ...
        "needs_visual_review": bool(images or english_score < 0.4),
...
def analyze_pdf(file_path: str) -> list[dict[str, Any]]:
    ...
    # Many Indian board papers place Hindi and English copies on adjacent pages.
    # Only mark a page as a likely duplicate when the following page is a strong
    # English question page with the same question range. This is intentionally
    # conservative: uncertain pages remain eligible for visual review.
