...
def _part_marker_y(page: fitz.Page, part_identifier: str, region: fitz.Rect) -> float | None:
    ...
    if not target:
        return None
    patterns = {target, f"({target})", f"{target})", f"({target}."}
    try:
...
def _best_visual_rect(page: fitz.Page, candidates: list[VisualAsset], question_region: fitz.Rect, part_identifier: str | None = None) -> fitz.Rect | None:
    if not candidates:
        return None
    ...
    for rect in components:
        ...
        if center_y < marker_y:
            continue
        if part == "a" and or_y is not None and center_y >= or_y:
            continue
...
def persist_source_visuals(pdf_path: str | Path, page_number: int, question_number: str, question_id: str, part_identifier: str | None = None) -> list[dict[str, Any]]:
    ...
    with fitz.open(str(pdf_path)) as document:
        ...
        if existing_row:
            ...
            return [existing_row]
        storage.create_question_asset(asset_id=asset_id, question_id=question_id, asset_type="visual", original_filename=output_path.name, file_path=str(output_path))
        return [{"asset_id": asset_id, "question_id": question_id, "asset_type": "visual", "original_filename": output_path.name, "file_path": str(output_path), "source_page": page_number, "source_bbox": tuple(rect)}]
