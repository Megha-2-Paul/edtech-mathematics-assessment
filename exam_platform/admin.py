...
def _save_uploaded_question_image(question_id):
    ...
    storage.create_question_asset(asset_id, question_id, "image", secure_filename(file.filename), str(path))
    return asset_id, f"/uploads/question_assets/{filename}", str(path)
...
def _copy_question_assets(source, new_question_id):
    ...
    for block in source.question_content:
        ...
        if block.type == "image" and block.metadata.get("local_asset_path"):
            ...
            if old_path.exists():
                new_path = QUESTION_ASSET_DIR / f"{new_question_id}_{uuid.uuid4().hex[:10]}{old_path.suffix.lower()}"
                shutil.copy2(old_path, new_path)
                asset_id = f"QASSET{uuid.uuid4().hex[:12].upper()}"
                storage.create_question_asset(asset_id, new_question_id, "image", old_path.name, str(new_path))
...
def _question_from_form(question_id, existing=None, preserve_images=True):
    ...
    q = Question(
        ...
        chapter=_form_value("chapter") or None, topic=_form_value("topic") or None,
        subtopic=_form_value("subtopic") or None, difficulty=_form_value("difficulty") or None,
        competency=_form_value("competency") or None, source=_form_value("source") or None,
...
def _form_context(question=None, is_edit=False):
    ...
    return {
        "question": question, "is_edit": is_edit,
        "subjects": storage.get_subjects(), "boards": BOARDS,
...
def _validate_question(q):
    if not q.question_content or not q.question_content[0].value:
    ...
    if not q.subject or not q.class_level or not q.board or not q.chapter:
...
def add_question():
    ...
    if request.method=="POST":
        question_id=_next_question_id(); q=_question_from_form(question_id,source,preserve_images=bool(source))
        try:
            uploaded=_save_uploaded_question_image(question_id)
            if uploaded:
                asset_id,url,path=uploaded; q.question_content.append(ContentBlock("image",url,asset_id=asset_id,metadata={"local_asset_path":path}))
            error=_validate_question(q)
            if error:
                flash(error,"error"); return render_template("teacher_question_form.html",**_form_context(q,False))
            storage.create_question(q); flash(f"Question {question_id} saved.","success"); return redirect(url_for("admin.questions"))
        except ValueError as exc:
            flash(str(exc),"error"); return render_template("teacher_question_form.html",**_form_context(q,False))
    if source:
        source=deepcopy(source); source.question_id="NEW"
...
def edit_question(question_id):
    ...
    if request.method=="POST":
        save_as_new=request.form.get("save_as_new")=="1"; new_id=_next_question_id() if save_as_new else question_id
        updated=_question_from_form(new_id,q,preserve_images=True)
        try:
            if save_as_new: updated.question_content=_copy_question_assets(q,new_id)
            uploaded=_save_uploaded_question_image(new_id)
            if uploaded:
                asset_id,url,path=uploaded; updated.question_content.append(ContentBlock("image",url,asset_id=asset_id,metadata={"local_asset_path":path}))
            error=_validate_question(updated)
            if error:
                flash(error,"error"); return render_template("teacher_question_form.html",**_form_context(updated,not save_as_new))
            if not save_as_new: storage.delete_question_assets(question_id)
            storage.create_question(updated)
            flash(f"Question {new_id} {'created as a new question' if save_as_new else 'updated'}.","success")
...
def add_test():
    if request.method=="POST":
        ...
        if not selected:
            flash("Select at least one active question.","error"); return render_template("teacher_test_form.html",questions=list(storage.questions.values()),form=request.form)
