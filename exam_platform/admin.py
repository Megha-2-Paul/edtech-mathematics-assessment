...
from .models import ContentBlock, Question, Test
...
def _save_uploaded_question_image(question_id):
    ...
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValueError("Question image must be JPG, JPEG, PNG or WEBP.")

    filename = f"{question_id}_{uuid.uuid4().hex[:10]}.{ext}"
    path = QUESTION_ASSET_DIR / filename
    ...
    for block in source.question_content:
        ...
        if block.type == "image" and block.metadata.get("local_asset_path"):
            ...
            if old_path.exists():
                new_path = (
                    QUESTION_ASSET_DIR
                    / f"{new_question_id}_{uuid.uuid4().hex[:10]}{old_path.suffix.lower()}"
                )
                shutil.copy2(old_path, new_path)
                block.asset_id = f"QASSET{uuid.uuid4().hex[:12].upper()}"
                block.value = f"/uploads/question_assets/{new_path.name}"
                block.metadata["local_asset_path"] = str(new_path)
        blocks.append(block)

    return blocks


def _persist_question_assets(question):
    for block in question.question_content:
        if (
            block.type == "image"
            and block.asset_id
            and block.metadata.get("local_asset_path")
        ):
            storage.create_question_asset(
                block.asset_id,
                question.question_id,
                "image",
                Path(block.metadata["local_asset_path"]).name,
                block.metadata["local_asset_path"],
            )


def _question_from_form(question_id, existing=None, preserve_images=True):
    question_type = _form_value("question_type", "mcq").lower()
    if question_type not in {item[0] for item in QUESTION_TYPES}:
        question_type = "mcq"
    ...
    choices = [
        _form_value(f"choice_{letter}")
        for letter in "ABCD"
    ]
    choices = [choice for choice in choices if choice]
    correct = _form_value("correct_answer").upper() or None
    content = [ContentBlock("text", _form_value("question_text"))]
    ...
    if (
        ...
        and not any(
            value == "1"
            for value in request.form.getlist("remove_existing_image")
        )
    ):
        content.extend(
            deepcopy(
                [
                    block
                    for block in existing.question_content
                    if block.type == "image"
                ]
            )
        )
    ...
    return Question(
        ...
        handwritten_upload_mode=_form_value(
        ...
        chapter=_form_value("chapter") or None,
        topic=_form_value("topic") or None,
        subtopic=_form_value("subtopic") or None,
        difficulty=_form_value("difficulty") or None,
        competency=_form_value("competency") or None,
        source=_form_value("source") or None,
...
def _form_context(question=None, is_edit=False):
    subject = question.subject if question else "Mathematics"
    board = question.board if question else "CBSE"
    class_level = question.class_level if question else 10
    ...
    return {
        ...
        "chapters": storage.get_chapters(board, class_level, subject),
        "competencies": storage.get_competencies(subject),
    }


def _validate_question(question):
    if not question.question_content or not question.question_content[0].value:
        return "Question text is required."

    if (
        not question.answer_choices
        or not question.correct_answer
        or question.correct_answer
        not in {chr(65 + i) for i in range(len(question.answer_choices))}
    ):
        return "Add answer choices and select a valid correct answer."

    if (
        not question.subject
        or not question.class_level
        or not question.board
        or not question.chapter
    ):
        return "Subject, board, class and chapter are required."
...
def api_chapters():
    return jsonify(
        [
            dict(item)
            for item in storage.get_chapters(
                request.args.get("board", "CBSE"),
                int(request.args.get("class_level", "10")),
                request.args.get("subject", "Mathematics"),
            )
        ]
    )
...
def api_competencies():
    return jsonify(
        [
            dict(item)
            for item in storage.get_competencies(
                request.args.get("subject", "Mathematics")
            )
        ]
    )
...
def questions():
    ...
    items = storage.get_all_questions()

    if query:
        items = [
            question
            for question in items
            if query in question.question_id.lower()
            or query in (question.chapter or "").lower()
            or query in (question.topic or "").lower()
            or query in str(question.question_content[0].value).lower()
        ]
    if question_type:
        items = [item for item in items if item.question_type == question_type]
    if board:
        items = [item for item in items if (item.board or "") == board]
    if class_level:
        items = [item for item in items if str(item.class_level or "") == class_level]
    if subject:
        items = [item for item in items if item.subject == subject]
    if chapter:
        items = [item for item in items if (item.chapter or "") == chapter]
    if difficulty:
        items = [item for item in items if (item.difficulty or "") == difficulty]
    if competency:
        items = [item for item in items if (item.competency or "") == competency]
    if status != "all":
...
def add_question():
    ...
    source = storage.get_question(clone_id) if clone_id else None

    if request.method == "POST":
        question_id = _next_question_id()
        question = _question_from_form(
            question_id,
            source,
        ...
        try:
            if source:
                question.question_content = [
                    question.question_content[0]
                ] + _copy_question_assets(source, question_id)

            uploaded = _save_uploaded_question_image(question_id)
            if uploaded:
                asset_id, url, path, original = uploaded
                question.question_content.append(
                    ContentBlock(
                        "image",
                        url,
                        asset_id=asset_id,
            ...
            if error:
                flash(error, "error")
                return render_template(
                    "teacher_question_form.html",
                    **_form_context(question, False),
                )

            storage.create_question(question)
            _persist_question_assets(question)
    ...
    if source:
        ...
        source.question_id = "NEW"

    return render_template(
        "teacher_question_form.html",
        **_form_context(source, False),
    )
...
def edit_question(question_id):
    ...
    if request.method == "POST":
        save_as_new = request.form.get("save_as_new") == "1"
        new_id = _next_question_id() if save_as_new else question_id
        updated = _question_from_form(
            new_id,
            question,
            preserve_images=not save_as_new,
        )

        try:
            if save_as_new:
                updated.question_content = [
                    updated.question_content[0]
                ] + _copy_question_assets(question, new_id)

            uploaded = _save_uploaded_question_image(new_id)
            if uploaded:
                asset_id, url, path, original = uploaded
                updated.question_content.append(
                    ContentBlock(
                        "image",
                        url,
                        asset_id=asset_id,
            ...
            if error:
                ...
                return render_template(
                    "teacher_question_form.html",
                    **_form_context(updated, not save_as_new),
                )

            if not save_as_new:
                storage.delete_question_assets(question_id)
            storage.create_question(updated)
            _persist_question_assets(updated)
            flash(
                f"Question {new_id} "
                f"{'created as a new question' if save_as_new else 'updated'}.",
                "success",
            )
            return redirect(
                url_for("admin.edit_question", question_id=new_id)
                if save_as_new
                else url_for("admin.questions")
            )
        except ValueError as exc:
            flash(str(exc), "error")
            return render_template(
...
def delete_question(question_id):
    ...
    flash(f"Question {question_id} deactivated.", "success")
    return redirect(url_for("admin.questions"))
...
def activate_question(question_id):
    if not storage.get_question(question_id):
        return "Question not found", 404
    storage.activate_question(question_id)
    flash(f"Question {question_id} activated.", "success")
    return redirect(url_for("admin.questions"))
...
def add_test():
    if request.method == "POST":
        selected = [
            ...
            if qid in storage.questions and storage.questions[qid].status == "active"
        ]

        if not selected:
            flash("Select at least one active question.", "error")
            return render_template(
    ...
    return render_template(
        ...
        questions=[
            question
            for question in storage.get_all_questions()
