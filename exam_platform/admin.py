...
def _question_from_form(question_id):
    question_type = request.form.get("question_type", "mcq").strip()
    answer_mode = "final_answer_selection_and_handwritten_upload" if question_type == "subjective" else "option_selection"
    ...
    return Question(
        ...
        question_content=content, answer_choices=choices, correct_answer=correct, marks=marks,
        ...
        chapter=request.form.get("chapter", "").strip() or None, topic=request.form.get("topic", "").strip() or None,
        subtopic=request.form.get("subtopic", "").strip() or None, difficulty=request.form.get("difficulty", "").strip() or None,
        competency=request.form.get("competency", "").strip() or None, source=request.form.get("source", "").strip() or None,
...
def questions():
    query = request.args.get("q", "").strip().lower(); question_type = request.args.get("type", "").strip(); board = request.args.get("board", "").strip(); class_level = request.args.get("class_level", "").strip()
...
def add_question():
    if request.method == "POST":
        question_id = _next_question_id(); q = _question_from_form(question_id)
        if not q.question_content[0].value:
            flash("Question text is required.", "error"); return render_template("teacher_question_form.html", question=q, is_edit=False)
        if not q.answer_choices or not q.correct_answer or q.correct_answer not in {chr(65 + i) for i in range(len(q.answer_choices))}:
            flash("Add answer choices and select a valid correct answer.", "error"); return render_template("teacher_question_form.html", question=q, is_edit=False)
...
def edit_question(question_id):
    ...
    if request.method == "POST":
        updated = _question_from_form(question_id)
        updated.status = q.status
        if not updated.question_content[0].value:
            flash("Question text is required.", "error"); return render_template("teacher_question_form.html", question=updated, is_edit=True)
        if not updated.answer_choices or not updated.correct_answer or updated.correct_answer not in {chr(65 + i) for i in range(len(updated.answer_choices))}:
            flash("Add answer choices and select a valid correct answer.", "error"); return render_template("teacher_question_form.html", question=updated, is_edit=True)
        storage.create_question(updated); flash(f"Question {question_id} updated.", "success"); return redirect(url_for("admin.questions"))
...
def delete_question(question_id):
    ...
    storage.delete_question(question_id); flash(f"Question {question_id} deactivated.", "success")
...
def activate_question(question_id):
    if not storage.get_question(question_id): return "Question not found", 404
    storage.activate_question(question_id); flash(f"Question {question_id} activated.", "success")
    return redirect(url_for("admin.questions"))
...
def add_test():
    if request.method == "POST":
        ...
        test_id = _next_test_id(); duration = int(request.form.get("duration_minutes", "60"))
        test = Test(test_id=test_id,title=request.form.get("title", "").strip() or test_id,subject=request.form.get("subject", "Mathematics").strip() or "Mathematics",class_level=int(request.form.get("class_level", "10")),duration_minutes=duration,total_marks=sum(storage.questions[qid].marks for qid in selected),questions=selected,status=request.form.get("status", "active"),board=request.form.get("board", "").strip() or None,test_date=request.form.get("test_date") or None,test_type=request.form.get("test_type", "weekly").strip() or "weekly")
