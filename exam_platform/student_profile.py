...
def build_student_profile(student_id):
    with engine.connect() as db:
        ...
        chapters = db.execute(
            text(
                """
                SELECT a.attempt_id, a.submitted_at, q.chapter, q.topic,
                       q.competency, q.marks, r.marks_awarded
                FROM responses r
                JOIN attempts a ON a.attempt_id = r.attempt_id
        ...
        history = db.execute(
            text(
                ...
                SELECT question_id, attempt_count, correct_count,
                       last_attempted_at, last_correct_at, last_marks_awarded,
                       last_error_summary
    ...
    for row in chapters:
        max_marks = _num(row["marks"])
        awarded = _num(row["marks_awarded"])

        for bucket, name in (
            (chapter_totals, row["chapter"] or "Unclassified"),
            (topic_totals, row["topic"] or "Unclassified"),
            (competency_totals, row["competency"] or "Unclassified"),
        ):
            bucket[name][0] += max_marks
            bucket[name][1] += awarded
            bucket[name][2] += 1
    ...
    score_history = [
        {
            ...
            "score": _num(attempt["score"]),
            "percentage": _num(attempt["percentage"]),
            "attempt_rate": _num(attempt["attempt_rate"]),
            "accuracy": _num(attempt["accuracy"]),
        }
    ...
    return {
        ...
        "previous": previous,
        "score_change": (
            round(latest["score"] - previous["score"], 2)
            if latest and previous
            else None
        ),
        "percentage_change": (
            round(latest["percentage"] - previous["percentage"], 2)
            if latest and previous
            else None
        ),
        "attempt_rate_change": (
            round(latest["attempt_rate"] - previous["attempt_rate"], 2)
            if latest and previous
            else None
        ),
        "accuracy_change": (
            round(latest["accuracy"] - previous["accuracy"], 2)
            if latest and previous
            else None
        ),
        "error_counts": [
