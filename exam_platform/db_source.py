...
def _question(row):
    ...
    return Question(
        ...
        row["answer_mode"],
        blocks,
        json.loads(row["answer_choices_json"]),
        row["correct_answer"],
        row["marks"],
        row["handwritten_upload_mode"],
        row["subject"],
        row["board"],
        row["class_level"],
        row["chapter"],
...
def _test(row):
    questions = (
        [r["question_id"] for r in row["question_rows"]]
        if "question_rows" in row
...
class DatabaseFirstStorage:
            ...
            for row in rows:
                links = db.execute(
                    text(
                        ...
                        "WHERE test_id=:id ORDER BY sequence_number"
                    ),
                    {"id": row["test_id"]},
                ).mappings().all()
                data = dict(row)
    ...
    def get_test(self, test_id):
        with engine.connect() as db:
            row = db.execute(
                text("SELECT * FROM tests WHERE test_id=:id"),
                {"id": test_id},
            ).mappings().first()
            if not row:
                return None
    ...
    def get_question(self, qid):
        with engine.connect() as db:
            row = db.execute(
                text("SELECT * FROM questions WHERE question_id=:id"),
                {"id": qid},
            ).mappings().first()
    ...
    def get_questions(self, qids):
        ...
        with engine.connect() as db:
            rows = db.execute(
                text(f"SELECT * FROM questions WHERE question_id IN ({placeholders})"),
                params,
            ).mappings().all()
    ...
    def get_attempt(self, aid):
        with engine.connect() as db:
            row = db.execute(
                text("SELECT * FROM attempts WHERE attempt_id=:id"),
                {"id": aid},
            ).mappings().first()
        ...
        with engine.connect() as db:
            row = db.execute(
                text(
                    ...
                    "WHERE student_id=:student AND test_id=:test "
                    "ORDER BY started_at DESC LIMIT 1"
                ),
                {"student": sid, "test": tid},
            ).mappings().first()
            return _attempt(row) if row else None
        ...
        with engine.connect() as db:
            return [
                ...
                for r in db.execute(
                    text(
                        "SELECT * FROM responses "
                        "WHERE attempt_id=:id ORDER BY response_id"
                    ),
                    {"id": aid},
                ).mappings()
            ]
