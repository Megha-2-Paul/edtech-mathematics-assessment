...
def _evaluation_errors(response_id):
    with engine.connect() as db:
        rows = db.execute(text("SELECT error_code, comment, marks_lost FROM evaluation_errors WHERE response_id=:id ORDER BY evaluation_error_id"), {"id": response_id}).mappings().all()
    return rows
...
def _evaluate_attempt(attempt_id):
    ...
    for response in responses:
        q = question_map.get(response.question_id)
        if not q:
            continue
        if response.answer_status == "answered":
            attempted += 1
        awarded += float(response.marks_awarded or 0)
    attempt.score = round(awarded, 2)
    attempt.percentage = round((awarded / total) * 100, 2) if total else 0
    attempt.attempt_rate = round((attempted / len(questions)) * 100, 2) if questions else 0
    correct_attempts = sum(1 for r in responses if r.is_correct is True)
    attempt.accuracy = round((correct_attempts / attempted) * 100, 2) if attempted else 0
...
def _save_response_evaluation(response, question, marks, error_codes, comment):
    ...
    if question.question_type == "mcq":
        is_correct = response.selected_answer == question.correct_answer if response.selected_answer else False
    else:
    ...
    with engine.begin() as db:
        ...
        marks_lost = max(0.0, float(question.marks) - marks)
        for code in error_codes:
            db.execute(text("INSERT INTO evaluation_errors(response_id,error_code,comment,marks_lost) VALUES(:response,:code,:comment,:lost)"), {"response": response.response_id, "code": code, "comment": comment or None, "lost": marks_lost if len(error_codes) == 1 else None})
...
def _update_question_history(attempt_id):
    ...
    with engine.begin() as db:
        for response in responses:
            errors = db.execute(text("SELECT error_code FROM evaluation_errors WHERE response_id=:id ORDER BY evaluation_error_id"), {"id": response.response_id}).scalars().all()
            ...
            db.execute(text("""INSERT INTO question_history(student_id,question_id,attempt_count,correct_count,last_attempted_at,last_correct_at,last_marks_awarded,last_error_summary)
                ...
                ON DUPLICATE KEY UPDATE attempt_count=attempt_count+1,correct_count=correct_count+VALUES(correct_count),last_attempted_at=VALUES(last_attempted_at),last_correct_at=IF(VALUES(correct_count)=1,VALUES(last_correct_at),last_correct_at),last_marks_awarded=VALUES(last_marks_awarded),last_error_summary=VALUES(last_error_summary)"""), {
                    "student": attempt.student_id,
                    "question": response.question_id,
                    "correct": 1 if response.is_correct else 0,
                    "attempted": attempt.submitted_at or datetime.now(),
                    "correct_at": attempt.submitted_at if response.is_correct else None,
...
def submissions():
    with engine.connect() as db:
        rows = db.execute(text("""SELECT a.attempt_id,a.student_id,a.test_id,a.started_at,a.submitted_at,a.status,a.score,a.percentage,t.title,s.name AS student_name
            FROM attempts a JOIN tests t ON t.test_id=a.test_id JOIN students s ON s.student_id=a.student_id
            WHERE a.status IN ('submitted','evaluated') ORDER BY COALESCE(a.submitted_at,a.started_at) DESC""")).mappings().all()
...
def evaluate(attempt_id):
    ...
    test = storage.get_test(attempt.test_id)
    questions = storage.get_questions(test.questions)
    responses = {r.question_id: r for r in storage.get_attempt_responses(attempt_id)}
    if request.method == "POST":
        for q in questions:
            ...
            if q.question_type == "mcq":
                marks = float(q.marks) if response.selected_answer == q.correct_answer else 0.0
                codes = []
            else:
                marks = request.form.get(f"marks_{q.question_id}", "0") or "0"
                codes = [c for c, _ in ERROR_CODES if c in request.form.getlist(f"errors_{q.question_id}")]
            comment = request.form.get(f"comment_{q.question_id}", "").strip()
            _save_response_evaluation(response, q, marks, codes, comment)
        attempt = _evaluate_attempt(attempt_id)
        attempt.status = "evaluated"
        storage.update_attempt(attempt)
        _update_question_history(attempt_id)
        flash("Evaluation saved and result calculated.", "success")
    ...
    for q in questions:
        ...
        if not r:
            response_data.append({"question":q,"response":None,"errors":[]})
        else:
            response_data.append({"question":q,"response":r,"errors":_evaluation_errors(r.response_id)})
...
def student_result(attempt_id):
    attempt = _attempt_or_404(attempt_id)
    if attempt.status not in {"evaluated", "submitted"}:
    ...
    for row in rows:
        error_map.setdefault(row["question_id"],[]).append(row)
    total=sum(float(q.marks) for q in questions)
    score=sum(float(response_map[q.question_id].marks_awarded or 0) for q in questions if q.question_id in response_map)
