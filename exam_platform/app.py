...
def test_listing():
    ...
    for test in tests:
        ...
        if attempt and attempt.status == AttemptStatus.SUBMITTED.value:
            ...
            test_status[test.test_id] = 'expired'
            ...
            if attempt_is_expired(attempt):
                ...
                test_status[test.test_id] = 'in_progress'
        else:
...
def test_instructions(test_id):
    ...
    if existing and existing.status == AttemptStatus.SUBMITTED.value:
        return redirect(url_for('test_listing'))
    if existing and existing.status == AttemptStatus.IN_PROGRESS.value and not attempt_is_expired(existing):
        return redirect(url_for('exam_interface', test_id=test_id, attempt_id=existing.attempt_id))
    questions = storage.get_questions(test.questions)
    return render_template('test_instructions.html', test=test, total_questions=len(questions))
...
def start_test(test_id):
    ...
    if existing:
        ...
        if existing.status == AttemptStatus.IN_PROGRESS.value and not attempt_is_expired(existing):
            session['attempt_id'] = existing.attempt_id
            return jsonify({
                "attempt_id": existing.attempt_id,
                "student_id": student_id,
                "resumed": True,
                "redirect_url": url_for('exam_interface', test_id=test_id, attempt_id=existing.attempt_id),
            })
        if existing.status == AttemptStatus.EXPIRED.value or attempt_is_expired(existing):
            return jsonify({"error": "Test time has expired", "status": "expired"}), 410
...
def exam_interface(test_id, attempt_id):
    ...
    questions = storage.get_questions(test.questions)
    return render_template('exam_interface.html', test=test, attempt=attempt, questions=questions, remaining_seconds=remaining_seconds)
...
def get_attempt_questions(attempt_id):
    ...
    for q in storage.get_questions(test.questions):
        ...
        question_data.append({
            ...
            "question_content": [{"type": c.type, "value": c.value} for c in q.question_content],
...
def save_response(attempt_id):
    ...
    test = storage.get_test(attempt.test_id)
    question = storage.get_question(question_id) if question_id else None
    if not question or question_id not in test.questions:
        return jsonify({"error": "Invalid question"}), 400
    if selected_answer not in {None, *[chr(65 + i) for i in range(len(question.answer_choices))]}:
        return jsonify({"error": "Invalid answer choice"}), 400
    existing = storage.get_response(attempt_id, question_id)
    status = AnswerStatus.ANSWERED.value if selected_answer else AnswerStatus.UNANSWERED.value
    if existing:
...
def upload_answer_image(attempt_id):
    ...
    if not question or question_id not in test.questions or question.handwritten_upload_mode == 'none':
    ...
    return jsonify({"image_id": image.image_id, "page_number": page_number, "filename": filename})
...
def get_submission_preview(attempt_id):
    ...
    return jsonify({
        ...
        "required_uploads_missing": required_uploads,
        "required_uploads_complete": len(required_uploads) == 0,
        "optional_uploads": optional_uploads,
    })
...
def submit_attempt(attempt_id):
    ...
    questions = storage.get_questions(test.questions)
    missing = [
        index for index, q in enumerate(questions, start=1)
        if q.handwritten_upload_mode == 'required' and not storage.get_attempt_images(attempt_id, q.question_id)
    ]
    if missing:
        return jsonify({"error": "Required handwritten work is missing", "missing_questions": missing}), 400
    attempt.submitted_at = datetime.now()
